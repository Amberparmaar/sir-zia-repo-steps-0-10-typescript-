...
type PersonEmploye = Employe & Person;

function personEmploye (emp: PersonEmploye){
    console.log( `This employe name is ${emp.name} , her id is ${emp.personId}`);
   
}


const emp : PersonEmploye = {
    name: "amber",
    personId: 789,
...
interface Benifits {
    weightLoss: string;
    preventFromCancer:boolean;
}

type Aboutcitrusfruit = CitrusFruit & Benifits;

let fruit : Aboutcitrusfruit = {
    name: "Lemon",
    colour: "yellow",
    isItHealthy: true,
    weightLoss: "yes",
...
function getFruit(fruit:Aboutcitrusfruit) {
